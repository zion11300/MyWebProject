...
window.loaded = function(){
	<% String location = request.getAttribute("location").toString().trim();
		String city = request.getAttribute("city").toString().trim();
		System.out.println(location+"/"+city);
	%>
	var location_value = <%=location%>;
	var city_value = <%=city%>;
	var location_option = document.getElementsById("location");
	var city_option = document.getElementsById("city");
	var location_idx = document.getElementsById("location").selectedIndex;
	var city_idx = location_option.selectedIndex;
	location_option.options[location_idx].selected = false;
	city_option.options[city_idx].selected = false;
	for(var i = 0 ; i < location_option.options.length ; i++){
		if(location_option.options[i].value == location_value){
			location.options[i].selected = true;
			break;
		}
	}
	
	for(var i = 0 ; i < city_option.options.length ; i++){
		if(city_option.options[i].value == city_value){
			city_option.options[i].selected = true;
			break;
		}	
	}
	...
	<div class="jumbotron" style="background-color:white;">
		<div class="container">
			...
			<hr>
						...
						<div class="row" style="border:0px solid white; padding: 20px;">
			<%
						int count = 0;
					%>
				<c:forEach var="sharedData" items="${sharedData}">
					
					<div class="col-sm-6 col-md-4">
						<div class="thumbnail">

							<img class="img-responsive" src="${sharedData.getFirstimageURL()}" width="300" height="400" 
							style="max-width: 300px; max-height: 400px; width:300px; height:400px" onclick="go(${sharedData.getContentid()})" >
							<script type="text/javascript">
							function go(test){
								location.href="./tourDetailResult?ContentID="+test;
								
							}
						</script>
							 <h4 style="text-align: center;">${sharedData.getTitle() }</h4>
						</div>
					</div>
					<%
						if (count == 3) { count=0;
